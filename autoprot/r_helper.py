...
def merge_data_from_r(
    ...
    ---------
    data_loc : str
        The location of the output data file generated by R.
    orig_df : pd.DataFrame
        The original DataFrame to merge with the output data.
    suffix : str
        Suffix to append to the column names of the R output DataFrame.
    locs_to_remove : list[str] | str | None, optional
        Locations of files to remove after merging. If None, no files are removed.
    return_cols : bool, optional
        If True, return the columns of the R output DataFrame.

    Returns:
    -------
    pd.DataFrame
        A DataFrame containing the merged data from the original DataFrame and the R output.
    list[str] | None
        If return_cols is True, returns the columns of the R output DataFrame excluding "UID".
    """
    # rename columns of r_df
    r_df.columns = [i + suffix if i != "UID" else i for i in r_df.columns]

    # join and retain the rows of the original df
    df = orig_df.join(r_df.set_index("UID"), on="UID", how="left")
    # drop UID again
    df.drop("UID", axis=1, inplace=True)

    if isinstance(locs_to_remove, list):
        for loc in locs_to_remove:
            # remove the file if it exists
            if os.path.isfile(loc):
                os.remove(loc)
    elif isinstance(locs_to_remove, str):
        # remove the file if it exists
        if os.path.isfile(locs_to_remove):
            os.remove(locs_to_remove)
    else:
        print("No locs_to_remove provided, skipping file removal.")

    if return_cols:
        return df, [x for x in r_df.columns if x != "UID"]
    return df
